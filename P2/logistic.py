...
def binary_train(X, y, w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    """
    """
    # Covert y from (0,1) to (-1,1) to adapt the gradient equation here
    #y[y==0]=-1
    for step in range(max_iterations):
        sig_term = sigmoid(-y * np.dot(X,wb.T)) # Shape (N,)
        yX = y.reshape(-1,1) * X # shape (N,D+1)
        gradient = np.dot(sig_term,yX)
        # GD
        wb = wb + step_size * gradient
    """
    """
    for step in range(max_iterations):
        sig_term = sigmoid(np.dot(X,wb.T)) -y # Shape (N,)
        gradient = np.dot(sig_term,X)
        # GD
        wb = wb - step_size * gradient
    # Break up wb to w and b
    w = wb[1:]
    b = wb[0]
    """
    for step in range(max_iterations):
...
def multinomial_train(X, y, C, 
    ...
    """
    for step in range(max_iterations):
        scores = np.dot(X,w.T) + b.reshape(1,-1) #shape (N,C)
        max_scores = np.max(scores,axis=1)
        scores_subtracted = scores - max_scores.reshape(-1,1)
        scores_exp = np.exp(scores_subtracted)
        denominator = np.sum(scores_exp,axis=1) # shape (N,)
...
def multinomial_predict(X, w, b):
    ...
    TODO: add your code here
    """
    # Use argmax, so the corresponding class has the largest score
    scores = np.dot(X,w.T) + b.reshape(1,-1)
...
def OVR_train(X, y, C, w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    for c in range(C):
        ...
        yy[y==c] = 1
        w[c,:],b[c] = binary_train(X, yy, w0=w[c,:], b0=b[c], step_size=step_size, max_iterations=max_iterations)
...
def OVR_predict(X, w, b):
    ...
    for c in range(C):
        predictions[:,c] = sigmoid(np.dot(X,w[c,:].T) + b[c])
