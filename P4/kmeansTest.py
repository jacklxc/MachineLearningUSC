...
def transform_image(image, code_vectors):
    ...
    reshaped = image.reshape(-1,shape2)
    square_dict = compute_square_distances(reshaped, code_vectors)
    membership = np.argmin(square_dict,axis=0)
    new_im = code_vectors[membership,:]
    # DONOT CHANGE CODE BELOW THIS LINE
    return new_im.reshape(shape0,shape1,shape2)
